#!/usr/bin/env bash
# GanjaloonReflector — clone of Reflector that issues GNJL against the
...
# Vanity-mining inputs — the address opens and closes on 0420.
mask=0xffff00000000000000000000000000000000ffff
target=0x0420000000000000000000000000000000000420

clone_predict GanjaloonReflector "$deployer" \
    "address,string" "$peg" "$symbol" \
    0x0000000000000000000000000000000000000000000000000000000346cee126
