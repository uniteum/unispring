...
# supply, variant) through the Lepton coinage. The caller sets the issue's vars
...
# decimals, supply); on-chain it is created by maker.issue(name, supply,
# variant), so the .txt encodes that call and the yml `via` points at the maker.
clone_predict "$clone" "$deployer" \
    "address,string,string,uint8,uint256" "$maker" "$name" "$symbol" "$decimals" "$supply" \
    "$variant" \
    -- "$maker" "issue(string,uint256,uint256)" "$name" "$supply" "$variant"
