 ...
 *           PositionManagerLookup — IAddressLookup for PositionManager
 *           ICoinage              — Coinage factory used to mint mimic tokens
 *           MimicoinageOwner      — recipient of every position NFT
 *
 * Usage:
...
contract MimicoinageDeploy is Script {
    ...
    function run() external {
        ...
        IAddressLookup positionManagerLookup = IAddressLookup(vm.envAddress("PositionManagerLookup"));
        ...
        console2.log("positionManagerLookup:", address(positionManagerLookup));
        console2.log("coinage              :", address(coinage));
        console2.log("owner                :", owner);

        bytes memory initCode = abi.encodePacked(
            type(Mimicoinage).creationCode, abi.encode(poolManagerLookup, positionManagerLookup, coinage, owner)
        );
